...
header('Location: mentor_workspace.php?team_id=' . $team_id);
exit();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_phase') {
        ...
        header('Location: mentor_view_project.php?team_id=' . $team_id . '&success_msg=' . urlencode('Phase updated successfully.'));
        exit();
        ...
        if ($message_text !== '') {
            ...
            foreach ($students as $student) {
                ...
                if ($student_id > 0) {
                    ...
                    $notif_link = 'mentor_view_project.php?team_id=' . $team_id;
                    $notif_stmt->bind_param('iss', $student_id, $notif_msg, $notif_link);
                    $notif_stmt->execute();
                    $notif_stmt->close();
                }
            }
            header('Location: mentor_view_project.php?team_id=' . $team_id . '&success_msg=' . urlencode('Team update posted successfully.'));
            exit();
        }
        header('Location: mentor_view_project.php?team_id=' . $team_id . '&error_msg=' . urlencode('Please enter an announcement before posting.'));
        exit();
        ...
        if ($log_id > 0) {
            ...
            header('Location: mentor_view_project.php?team_id=' . $team_id . '&success_msg=' . urlencode('Daily log review saved.'));
            exit();
        }
        header('Location: mentor_view_project.php?team_id=' . $team_id . '&error_msg=' . urlencode('Please choose a log entry to review.'));
        exit();
        ...
        if ($student_id && $internship_id && $rating > 0 && $phase !== '') {
            ...
            header('Location: mentor_view_project.php?team_id=' . $team_id . '&success_msg=' . urlencode('Feedback submitted successfully.'));
            exit();
        }
        header('Location: mentor_view_project.php?team_id=' . $team_id . '&error_msg=' . urlencode('Please complete the feedback form.'));
        exit();
        ...
        if ($student_id && $internship_id && $reason !== '') {
            ...
            if ($app_row) {
                ...
                header('Location: mentor_view_project.php?team_id=' . $team_id . '&success_msg=' . urlencode('Dropout request submitted successfully.'));
                exit();
            }
        }
        header('Location: mentor_view_project.php?team_id=' . $team_id . '&error_msg=' . urlencode('Please complete the dropout request form.'));
        exit();
    }
}
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
      <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        ...
        <div class="text-sm text-slate-500">
          <a href="mentor_dashboard.php" class="text-blue-600 font-semibold">← Back to dashboard</a>
        </div>
      </div>
        ...
        <div class="space-y-6">
          ...
          <div class="rounded-xl border border-slate-200 p-5">
            <div class="flex items-center justify-between">
              ...
              <a href="mentor_daily_logs.php" class="text-sm font-semibold text-blue-600">Open review center</a>
            </div>
            <?php if (empty($logs)): ?>
