...
if ($team_id <= 0 || empty($subject) || empty($message) || empty($student_id_raw)) {
    ...
    if (isset($_POST['ajax'])) {
        ...
        header("Location: notifications.php?error_msg=" . urlencode($err));
    }
    exit();
}
...
if (isset($_FILES['attachment']) && $_FILES['attachment']['error'] !== UPLOAD_ERR_NO_FILE) {
    ...
    if ($attachment_res === false) {
        if (isset($_POST['ajax'])) {
            ...
            header("Location: notifications.php?error_msg=" . urlencode($upload_err));
        }
        exit();
    }
...
if (isset($_POST['ajax'])) {
    ...
    if ($email_failed) {
        header("Location: notifications.php?success_msg=" . urlencode("Dashboard notification sent. Email could not be sent (check mail server setup)."));
    } else {
        header("Location: notifications.php?success_msg=" . urlencode("Notification sent successfully."));
    }
}
exit();
