...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'post_update') {
        ...
        if ($message_text === '') {
            header('Location: students.php?team_id=' . $team_id . '&team_name=' . urlencode($team['team_name']) . '&tab=discussion&error_msg=' . urlencode('Please enter a message before posting.'));
            exit();
        }
        ...
        if (!empty($student_ids)) {
            ...
            $notify_link = 'students.php?team_id=' . $team_id . '&team_name=' . urlencode($team['team_name']);
        ...
        header('Location: students.php?team_id=' . $team_id . '&team_name=' . urlencode($team['team_name']) . '&tab=discussion&success_msg=' . urlencode('Team update posted successfully.'));
        exit();
    }
    ...
    if ($action === 'send_notification') {
        ...
        if ($subject === '' || $message === '') {
            header('Location: students.php?team_id=' . $team_id . '&team_name=' . urlencode($team['team_name']) . '&tab=discussion&error_msg=' . urlencode('Please complete the notification details.'));
            exit();
        }
        ...
        header('Location: students.php?team_id=' . $team_id . '&team_name=' . urlencode($team['team_name']) . '&tab=discussion&success_msg=' . urlencode('Notification sent to the team.'));
        exit();
    }
    ...
    if ($action === 'raise_dropout') {
        ...
        if ($student_id <= 0 || $internship_id <= 0 || $reason === '') {
            header('Location: students.php?team_id=' . $team_id . '&team_name=' . urlencode($team['team_name']) . '&tab=students&error_msg=' . urlencode('Please complete the dropout request form.'));
            exit();
        }
        ...
        header('Location: students.php?team_id=' . $team_id . '&team_name=' . urlencode($team['team_name']) . '&tab=students&success_msg=' . urlencode('Dropout request submitted successfully.'));
        exit();
    }
}
...
$action_html = '<a href="projects.php" class="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50 transition-all"><span class="material-symbols-outlined text-[18px]">arrow_back</span>Back to Projects</a>';
            ...
            <div class="space-y-2">
                ...
                <div class="data-grid-row grid grid-cols-[1.2fr_1.4fr_1fr_100px_100px_250px] gap-4 items-center rounded-xl border border-slate-100 bg-white hover:bg-slate-50 transition-colors px-4 py-3.5">
                    ...
                    <div class="text-center flex items-center justify-center gap-2" style="min-width:250px">
                        <a href="daily_logs.php?student_id=<?= $student['id'] ?>" class="inline-flex items-center gap-1 rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-blue-700 transition-colors">
...
<div id="panel-logs" class="ws-panel<?= $active_tab === 'logs' ? ' active' : '' ?>" role="tabpanel">
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex items-center justify-between mb-5">
            ...
            <a href="daily_logs.php" class="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3.5 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors shadow-sm">
            ...
            <div class="space-y-2">
                ...
                <div class="data-grid-row grid grid-cols-[1.1fr_0.8fr_1.8fr_0.6fr_auto_auto] gap-4 items-center rounded-xl border border-slate-100 bg-white hover:bg-slate-50 transition-colors px-4 py-3.5">
                    ...
                    <div class="text-center" style="min-width:80px">
                        <a href="daily_logs.php" class="inline-flex items-center gap-1 rounded-lg border border-blue-200 bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700 hover:bg-blue-100 transition-colors">
...
<div id="dropout-modal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-150">
        ...
        <form action="students.php?team_id=<?= $team_id ?>" method="POST" class="p-6 space-y-4" onsubmit="return confirm('Submit this dropout request for administration review?');">
