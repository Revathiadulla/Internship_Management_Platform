...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_timeline') {
    ...
    if ($success) {
        header("Location: /IMP/coordinator/internships.php?success=" . urlencode("Timeline workflow updated successfully!"));
        exit();
    } else {
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    if ($critical_blockers === 0 && $questions_count > 0) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            header("Location: /IMP/coordinator/internships.php?success=" . urlencode("Project posting archived" . ($questions_count > 0 ? " (test questions removed)." : ".")));
            exit();
        } else {
        ...
        mysqli_stmt_bind_param($stmt, "i", $id);
        if (mysqli_stmt_execute($stmt)) {
            header("Location: /IMP/coordinator/internships.php?success=" . urlencode("Project posting archived to preserve student records."));
            exit();
        } else {
            $error_msg = "Error archiving posting: " . mysqli_error($conn);
        }
        ...
        mysqli_stmt_bind_param($stmt, "i", $id);
        if (mysqli_stmt_execute($stmt)) {
            header("Location: /IMP/coordinator/internships.php?success=" . urlencode("Project posting removed."));
            exit();
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create') {
    ...
    if (empty($title) || empty($project_title) || empty($duration) || empty($mode) || empty($technology_stack) || empty($description) || $project_type_id <= 0 || $project_subtype_id <= 0) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            header("Location: /IMP/coordinator/internships.php?success=" . urlencode("Project posting submitted for Admin approval!"));
            exit();
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'edit') {
    ...
    if (empty($title) || empty($project_title) || empty($duration) || empty($mode) || empty($technology_stack) || empty($description) || $project_type_id <= 0 || $project_subtype_id <= 0) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            header("Location: /IMP/coordinator/internships.php?success=" . urlencode("Project posting updated successfully!"));
            exit();
        } else {
...
<body class="bg-gray-100 text-gray-800">
        ...
        <aside class="fixed left-0 top-0 h-screen w-60 z-50 bg-white border-r border-gray-200 flex flex-col py-6">
                ...
                <nav class="flex-1 space-y-0.5 px-3">
                        ...
                        <a href="/IMP/coordinator/internships.php" class="flex items-center gap-3 bg-blue-50 text-blue-700 border-l-4 border-blue-600 px-3 py-2.5 rounded-r-lg text-sm font-semibold">
                ...
                <div class="border-t border-gray-200 pt-3 px-3 space-y-0.5">
                        ...
                        <a href="/IMP/logout.php" class="flex items-center gap-3 text-red-650 px-3 py-2.5 rounded-lg text-sm font-medium hover:bg-red-50 transition-colors">
        ...
        <main class="ml-60 flex flex-col min-h-screen">
                ...
                <header class="w-full sticky top-0 z-40 bg-white border-b border-gray-200 shadow-sm flex items-center justify-between px-8 py-3 font-sans antialiased text-sm">
                        ...
                        <div class="flex items-center gap-6">
                                <!-- Notifications Bell -->
                                <a href="/IMP/coordinator/notifications.php" class="p-2 text-gray-500 hover:bg-gray-50 transition-colors rounded-full relative">
                ...
                <div class="flex-1 p-8 space-y-6">
                        ...
                        <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                            <form method="GET" action="/IMP/coordinator/internships.php" class="flex gap-2 max-w-md">
                                ...
                                <?php if (!empty($search)): ?>
                                    <a href="/IMP/coordinator/internships.php" class="bg-gray-100 hover:bg-gray-200 border border-gray-200 text-gray-700 px-3 py-2 rounded-lg text-xs font-bold flex items-center justify-center">Reset</a>
                                <?php endif; ?>
                            </form>
                        </div>
                                                    ...
                                                    ?>
                                                        <tr class="hover:bg-gray-50 transition-colors">
                                                                ...
                                                                <td class="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                                                                    ...
                                                                    <a href="/IMP/coordinator/internships.php?action=delete&id=<?php echo $item['id']; ?>" onclick="return confirm('Are you sure you want to delete this project posting?');" class="text-red-600 hover:text-red-800 font-bold text-xs">Delete</a>
                                                                </td>
                                                        </tr>
                                                    <?php endforeach; ?>
                                                <?php endif; ?>
        ...
        <div id="internship-modal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
                <div class="bg-white rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                        ...
                        <form id="internship-form" method="POST" action="/IMP/coordinator/internships.php">
        ...
        <div id="timeline-modal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
                <div class="bg-white rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                        ...
                        <form id="timeline-form" method="POST" action="/IMP/coordinator/internships.php">
        ...
        <script>
                ...
                let currentSubtypes = [];
                 ...
                 function updateSubtypes(selectedType, selectedSubtype = '') {
                         ...
                         fetch('/IMP/includes/project_category_api.php?action=get_subtypes&type_id=' + encodeURIComponent(typeId))
                ...
                function openTimelineModal(item) {
                        ...
                        fetch(`/IMP/coordinator/internships.php?action=get_timeline&id=${item.id}`)
                                .then(response => response.json())
                                .then(phases => {
                                        if (phases.length === 0) {
                                                timelinePhasesContainer.innerHTML = `
