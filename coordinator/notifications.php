...
include __DIR__ . '/../includes/ensure_extended_schema.php';
...
<body class="bg-gray-100 text-gray-800">
    ...
    <aside class="fixed left-0 top-0 h-screen w-60 z-50 bg-white border-r border-gray-200 flex flex-col py-6">
        ...
        <nav class="flex-1 space-y-0.5 px-3">
            ...
            <a href="/IMP/coordinator/notifications.php" class="flex items-center gap-3 bg-blue-50 text-blue-700 border-l-4 border-blue-600 px-3 py-2.5 rounded-r-lg text-sm font-bold">
    ...
    <main class="ml-60 flex flex-col min-h-screen">
                    ...
                    <div id="notif-dropdown" class="hidden absolute right-0 mt-2 w-80 bg-white border border-gray-200 rounded-xl shadow-lg py-2 z-50">
                        <div class="px-4 py-3 border-b border-gray-100">
                            <div class="flex items-center justify-between gap-2">
                                ...
                                <a href="/IMP/coordinator/notifications.php?tab=inbox" class="text-[10px] font-bold text-blue-600 hover:text-blue-800">View all</a>
                            </div>
                        </div>
                        ...
                        <?php else: ?>
                            <?php foreach ($latest_notifications as $notif): ?>
                                <a href="/IMP/coordinator/notifications.php?tab=inbox" class="block px-4 py-3 hover:bg-gray-50 transition-colors border-b border-gray-100">
                                    <div class="flex items-center justify-between gap-2">
        ...
        <div class="flex-1 p-8 space-y-6 max-w-6xl w-full mx-auto">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-gray-200 pb-5">
                ...
                <a href="/IMP/coordinator/notifications.php?tab=compose" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-sm font-semibold transition-all shadow-sm">
            ...
            <div class="flex flex-wrap gap-2 overflow-x-auto pb-2">
                ...
                ?>
                    <a href="/IMP/coordinator/notifications.php?tab=<?php echo $key; ?>" class="px-4 py-2 rounded-full text-sm font-semibold transition-all border border-gray-200 <?php echo $active_class; ?>"><?php echo $label; ?></a>
                <?php endforeach; ?>
            </div>
                    ...
                    <div id="notifications-container" class="space-y-4">
                        <?php if (!empty($inbox_notifications)): ?>
                            ...
                            ?>
                                ...
                                <?php if ($notif_link): ?>
                                <a href="mark_notification_read.php?action=read_redirect&id=<?php echo $row['id']; ?>&fallback=/IMP/coordinator/notifications.php" class="notification-card block bg-white rounded-2xl border <?php echo $is_read ? 'border-gray-200' : 'border-blue-200 shadow-sm'; ?> p-5 transition-all flex items-start gap-4 hover:border-blue-400 cursor-pointer" data-id="<?php echo $row['id']; ?>" data-type="<?php echo htmlspecialchars(strtolower($type)); ?>" data-read="<?php echo $is_read ? 'true' : 'false'; ?>">
                                <?php else: ?>
            ...
            <?php elseif ($active_tab === 'compose'): ?>
                <div class="bg-white border border-gray-200 rounded-3xl shadow-sm p-6">
                    ...
                    <form method="POST" action="/IMP/includes/send_notification.php" enctype="multipart/form-data" class="space-y-5">
