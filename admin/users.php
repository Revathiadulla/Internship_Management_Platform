...
<body class="min-h-screen flex flex-col font-sans antialiased">
    ...
    <main class="flex-1 p-8 overflow-y-auto bg-gray-50">
      <div class="max-w-6xl mx-auto space-y-6">
        ...
        <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col md:flex-row gap-4 items-center">
          <form method="GET" action="users.php" class="w-full flex flex-col md:flex-row gap-4">
            ...
            <div class="flex gap-2">
              ...
              <?php if (!empty($search) || !empty($role_filter)): ?>
                <a href="users.php" class="bg-gray-100 hover:bg-gray-200 border border-gray-200 text-gray-700 px-4 py-2 rounded-lg text-xs font-bold flex items-center justify-center transition-colors">Reset</a>
              <?php endif; ?>
            </div>
          </form>
        </div>
                  ...
                  ?>
                    <tr class="hover:bg-gray-50/50">
                      ...
                      <td class="px-6 py-4 text-right space-x-2">
                        ...
                        <?php if (strtolower($user['status'] ?? 'active') === 'inactive'): ?>
                          <a href="users.php?action=restore&id=<?php echo $user['id']; ?>" class="text-emerald-600 hover:text-emerald-800 text-xs font-bold" onclick="return confirm('Restore this user?')">Restore</a>
                        <?php else: ?>
                          <a href="users.php?action=delete&id=<?php echo $user['id']; ?>" class="text-red-600 hover:text-red-800 text-xs font-bold" onclick="return confirm('Are you sure you want to deactivate this user?')">Delete</a>
                        <?php endif; ?>
                      </td>
                    </tr>
                  <?php endforeach; ?>
          ...
          <?php if ($total_pages > 1): ?>
            <div class="px-6 py-4 border-t border-gray-100 flex justify-between items-center text-xs">
              ...
              <div class="flex gap-1.5">
                <?php if ($page > 1): ?>
                  <a href="users.php?page=<?php echo $page - 1; ?>&search=<?php echo urlencode($search); ?>&role=<?php echo urlencode($role_filter); ?>" class="px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-gray-600 font-bold hover:bg-gray-50 transition-colors">Prev</a>
                <?php endif; ?>
                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                  <a href="users.php?page=<?php echo $i; ?>&search=<?php echo urlencode($search); ?>&role=<?php echo urlencode($role_filter); ?>" class="px-3 py-1.5 border rounded-lg font-bold transition-colors <?php echo $i === $page ? 'bg-[#003ea8] border-[#003ea8] text-white' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'; ?>"><?php echo $i; ?></a>
                <?php endfor; ?>
                <?php if ($page < $total_pages): ?>
                  <a href="users.php?page=<?php echo $page + 1; ?>&search=<?php echo urlencode($search); ?>&role=<?php echo urlencode($role_filter); ?>" class="px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-gray-600 font-bold hover:bg-gray-50 transition-colors">Next</a>
                <?php endif; ?>
              </div>
            </div>
          <?php endif; ?>
  ...
  <script>
    ...
    async function viewUserProfile(userId, role) {
      try {
        const response = await fetch(`users.php?action=get_profile&id=${userId}`);
