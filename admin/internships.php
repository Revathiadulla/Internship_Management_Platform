...
<body class="min-h-screen flex flex-col font-sans antialiased">
    ...
    <main class="flex-1 p-8 overflow-y-auto bg-gray-50">
      <div class="max-w-6xl mx-auto space-y-6">
        ...
        <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
          <form method="GET" action="internships.php" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
            <div class="relative">
            ...
            <div class="flex gap-2 justify-end">
              ...
              <?php if (!empty($search) || !empty($status_filter) || !empty($mode_filter)): ?>
                <a href="internships.php" class="bg-gray-100 hover:bg-gray-200 border border-gray-200 text-gray-700 px-4 py-2 rounded-lg text-xs font-bold flex items-center justify-center transition-colors">Reset</a>
              <?php endif; ?>
            </div>
          </form>
        </div>
              ...
              <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 flex flex-col justify-between hover:shadow-md transition-shadow gap-4">
                ...
                <div class="pt-4 border-t border-gray-100 flex flex-wrap gap-1.5 justify-end items-center">
                  ...
                  <?php if (in_array(strtolower($item['status']), ['pending approval', 'changes requested', 'new'])): ?>
                    <a href="internships.php?action=approve&id=<?php echo $item['id']; ?>" class="px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white rounded-lg text-xs font-bold transition-colors">Approve</a>
                    <a href="internships.php?action=archive&id=<?php echo $item['id']; ?>" onclick="return confirm('Are you sure you want to archive this posting?')" class="px-3 py-1.5 bg-gray-600 hover:bg-gray-700 text-white rounded-lg text-xs font-bold transition-colors">Archive</a>
                  <?php elseif (strtolower($item['status']) === 'archived'): ?>
                    <a href="internships.php?action=unarchive&id=<?php echo $item['id']; ?>" class="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold transition-colors">Unarchive</a>
                  <?php endif; ?>
                </div>
              </div>
            <?php endforeach; ?>
  ...
  <script>
    ...
    async function fetchInternshipDetails(id) {
      try {
        const response = await fetch(`internships.php?ajax=details&id=${encodeURIComponent(id)}`);
  ...
  <div id="delete-modal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden">
      <div class="p-6 text-center space-y-4">
        ...
        <form method="POST" action="internships.php" class="flex gap-3 justify-center pt-2">
