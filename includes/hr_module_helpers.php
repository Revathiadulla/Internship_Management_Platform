...
.profile-menu .dropdown.show { display: block !important; }
...
.notif-menu .dropdown.show { display: block !important; }
</style>
...
<nav class="flex-1 space-y-1 px-4">';
    foreach ($visible as [$label, $href, $icon, $key]) {
        $is_active = ($active === $key) || ($key === 'workflows' && $active === 'archived_applications');
        $class = $is_active
...
function module_topbar(string $active, string $action_html = '', bool $show_search = true): string {
    ...
    if ($is_mentor || $is_hr) {
        ...
        $notif_html = '<div class="notif-menu relative">
            <button type="button" class="notif-menu-toggle relative rounded-full p-2 text-slate-500 hover:bg-slate-50" title="Notifications Dropdown">
    ...
    return '<div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">' . $search_html . '<div class="flex items-center gap-3">' . $action_html . $notif_html . '<div class="profile-menu relative">
            <button type="button" class="profile-menu-toggle flex items-center gap-3 rounded-lg px-2 py-1.5 hover:bg-slate-50">
                ' . $avatar_html . '
...
// Profile and Notification click toggling
document.addEventListener("DOMContentLoaded", () => {
    ...
    const notifToggle = document.querySelector(".notif-menu-toggle");
    const notifDropdown = document.querySelector(".notif-menu .dropdown");
    ...
    // Close on selecting a menu item
    document.querySelectorAll(".profile-menu .dropdown a, .notif-menu .dropdown a").forEach(link => {
