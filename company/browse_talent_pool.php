...
$where = ["(
    LOWER(a.status) IN ('completed', 'internship completed') OR 
    LOWER(a.certificate_status) IN ('issued', 'completed', 'generated') OR
    LOWER(c.current_status) IN ('completed', 'internship completed', 'certificate issued')
) AND LOWER(c.current_status) NOT IN ('applied', 'hr review', 'hod approved', 'selected', 'project assigned', 'active internship', 'rejected', 'shortlisted', 'pending')"];
...
if ($candidates_res) {
    while ($row = mysqli_fetch_assoc($candidates_res)) {
        ...
        // Check hiring request status
        $check_h = mysqli_query($conn, "SELECT id FROM hiring_requests WHERE company_id = $company_id AND student_id = $cand_user_id LIMIT 1");
        $row['is_hiring_requested'] = ($check_h && mysqli_num_rows($check_h) > 0);
...
<body class="min-h-screen flex font-sans">
    ...
    <aside class="w-64 bg-white border-r border-gray-200 p-6 flex flex-col fixed h-screen z-50">
        ...
        <nav class="flex-1 space-y-1">
            <a href="/IMP/company/dashboard.php" class="sidebar-link">
                <span class="material-symbols-outlined text-xl">dashboard</span>
                Hiring Overview
            </a>
            <a href="/IMP/company/browse_talent_pool.php" class="sidebar-link active">
                <span class="material-symbols-outlined text-xl">person_search</span>
                Browse Talent Pool
            </a>
            <a href="/IMP/company/subscription.php" class="sidebar-link">
        ...
        <div class="mt-auto pt-6 border-t border-gray-100">
            <a href="/IMP/logout.php" class="sidebar-link text-red-600 hover:bg-red-50">
        ...
        <div class="max-w-7xl mx-auto space-y-8">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button onclick="window.location.href='/IMP/company/browse_talent_pool.php?filter=shortlist'" class="bg-white border border-gray-200 text-gray-700 px-5 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-gray-50 transition-all shadow-sm">
            ...
            <form method="GET" action="browse_talent_pool.php" class="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-6">
                <?php if ($filter_shortlist): ?>
                    ...
                    <div class="flex items-center justify-between bg-amber-50 border border-amber-100 rounded-xl px-4 py-2 text-xs font-bold text-amber-800">
                        ...
                        <a href="/IMP/company/browse_talent_pool.php" class="text-blue-600 hover:underline">Show All</a>
                    </div>
                <?php endif; ?>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    ...
                    <a href="/IMP/company/browse_talent_pool.php" class="bg-gray-900 text-white py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-gray-800 transition-all flex items-center justify-center text-center">Clear</a>
                </div>
            </form>
                ...
                <?php if (empty($candidates)): ?>
                    <div class="col-span-full bg-white rounded-2xl border border-gray-200 p-16 text-center text-gray-500 shadow-sm">
                        <div class="flex flex-col items-center justify-center py-4 text-center">
                            ...
                            <a href="/IMP/company/browse_talent_pool.php" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg shadow-sm transition-all hover:shadow-md">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow overflow-hidden group flex flex-col justify-between">
                        <div class="p-6">
                            ...
                            <div class="flex flex-col gap-2 pt-6 border-t border-gray-50">
                                <button onclick="toggleShortlist(this, <?php echo $cand['user_id']; ?>)" class="w-full text-white py-2.5 rounded-xl text-xs font-bold transition-all shadow-md <?php echo $cand['is_shortlisted'] ? 'bg-amber-600 hover:bg-amber-700' : 'bg-gray-900 hover:bg-gray-800'; ?>">
                                    <?php echo $cand['is_shortlisted'] ? 'Shortlisted' : 'Shortlist Candidate'; ?>
                                </button>
                                <button onclick="sendHiringRequest(this, <?php echo $cand['user_id']; ?>)" class="w-full text-white py-2.5 rounded-xl text-xs font-bold transition-all shadow-md <?php echo $cand['is_hiring_requested'] ? 'bg-emerald-600 pointer-events-none' : 'bg-blue-600 hover:bg-blue-700'; ?>">
                                    <?php echo $cand['is_hiring_requested'] ? 'Hiring Request Sent' : 'Send Hiring Request'; ?>
                                </button>
    ...
    <script>
        ...
        async function sendHiringRequest(btn, candidateId) {
            const formData = new FormData();
            formData.append('student_id', candidateId);

            try {
                const res = await fetch('send_hiring_request_api.php', {
                    method: 'POST',
                    body: formData
                });
                const data = await res.json();
                if (data.success) {
                    showToast(data.message);
                    btn.innerText = 'Hiring Request Sent';
                    btn.classList.remove('bg-blue-600', 'hover:bg-blue-700');
                    btn.classList.add('bg-emerald-600', 'pointer-events-none');
                } else {
                    alert(data.message || 'Error sending hiring request.');
                }
            } catch(e) {
                console.error(e);
