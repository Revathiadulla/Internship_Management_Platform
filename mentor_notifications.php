    ...
    <div id="notifications-container" class="space-y-4">
        <?php if ($total_notifications > 0): ?>
            ...
            ?>
                ...
                <?php if ($notif_link): ?>
                <a href="mark_notification_read.php?action=read_redirect&id=<?php echo $row['id']; ?>&fallback=mentor_notifications.php&source=global" class="notification-card block bg-white rounded-2xl border <?php echo $is_read ? 'border-slate-100' : 'border-blue-100 shadow-sm'; ?> p-5 transition-all flex items-start gap-4 hover:border-blue-300 cursor-pointer"
        ...
        if (modalForm) {
            modalForm.addEventListener('submit', async function(e) {
                ...
                try {
                    ...
                    const response = await fetch('mentor_send_notification.php', {
                        method: 'POST',
                        body: formData
                    });
