...
if (!$documents_verified) {
    header("Location: applicant_detail.php?app_id=$app_id&error=unverified_docs");
    exit();
}

if (!is_exam_sent_status($app['status'])) {
    header("Location: applicant_detail.php?app_id=$app_id&error=requires_exam_mail");
    exit();
}

if (!$is_pursuing) {
    header("Location: applicant_detail.php?app_id=$app_id&error=not_pursuing");
    exit();
}
...
if (empty($hod_email)) {
    header("Location: applicant_detail.php?app_id=$app_id&error=no_hod_email");
    exit();
}
...
header("Location: applicant_detail.php?app_id=$app_id&success=forwarded_to_hod");
exit();
