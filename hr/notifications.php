...
if (!in_array($user_role, ['admin', 'hr'], true)) {
    ...
    if ($user_role === 'mentor') header('Location: mentor/notifications.php');
...
<body class="bg-gray-100 text-gray-800 transition-colors duration-250">
    <div class="flex min-h-screen">
        <!-- SideNavBar -->
        <?php 
            include_once __DIR__ . '/../includes/hr_module_helpers.php'; hr_sidebar('notifications');
        ?>
                ...
                <div class="flex items-center gap-4">
                    <div class="relative" id="profile-container">
                        ...
                        <div id="profile-dropdown" class="hidden absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-xl shadow-lg py-2 z-50">
                            <a href="<?php echo ($user_role === 'hr') ? 'hr_dashboard.php' : 'dashboard.php'; ?>" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-blue-600 transition-colors">
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            singleReadButtons.forEach(btn => {
                btn.addEventListener('click', async () => {
                    ...
                    try {
                        const response = await fetch(`/IMP/mark_notification_read.php?action=read&id=${notifId}&source=global&receiver_role=<?php echo $role_filter; ?>`);
            ...
            if (markAllButton) {
                markAllButton.addEventListener('click', async () => {
                    try {
                        const response = await fetch('/IMP/mark_notification_read.php?action=read_all&source=global&receiver_role=<?php echo $role_filter; ?>');
                        const data = await response.json();
                        if (data.success) {
                            cards.forEach(card => {
                                card.setAttribute("data-read", "true");
            ...
            if (clearReadButton) {
                clearReadButton.addEventListener('click', async () => {
                    ...
                    try {
                        const response = await fetch('/IMP/mark_notification_read.php?action=delete_read&source=global&receiver_role=<?php echo $role_filter; ?>');
                        const data = await response.json();
                        if (data.success) {
