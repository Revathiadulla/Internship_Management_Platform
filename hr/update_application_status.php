...
$allowed_statuses = [];
if ($user_role === 'hr') {
    $allowed_statuses = ['Applied', 'Shortlisted', 'Exam Qualified', 'HOD Pending', 'HOD Approved', 'Selected', 'Rejected', 'Exam Link Sent', 'Exam Completed', 'HR Review'];
} elseif ($user_role === 'coordinator' || $user_role === 'admin') {
    $allowed_statuses = ['Applied', 'Shortlisted', 'Exam Qualified', 'HOD Pending', 'HOD Approved', 'Selected', 'Project Assigned', 'Active Intern', 'Rejected', 'Completed', 'Exam Link Sent', 'Exam Completed', 'HR Review'];
}
...
if ($new_status === 'Selected') {
    ...
    if (($app['aadhaar_status'] ?? '') !== 'verified' || ($app['pan_status'] ?? '') !== 'verified') {
        echo json_encode(['success' => false, 'message' => 'Cannot select this candidate. Documents (Aadhaar/PAN) must be verified first.']);
        exit();
    }
    ...
    if ($is_pursuing_be) {
        ...
        if (!in_array($old_status_key, ['exam_link_sent', 'exam_sent', 'exam_mail_sent', 'shortlisted', 'hod_approved', 'selected'], true)) {
            echo json_encode(['success' => false, 'message' => 'Cannot select this candidate before the exam is sent or shortlisted.']);
            exit();
        }
    }
}
...
if ($new_status === 'HOD Pending') {
    ...
    $approve_url = rtrim($base_url, '/') . '/hod/hod_approval_action.php?application_id=' . urlencode($app_id) . '&action=approve&token=' . urlencode($hod_token);
    $reject_url  = rtrim($base_url, '/') . '/hod/hod_approval_action.php?application_id=' . urlencode($app_id) . '&action=reject&token=' . urlencode($hod_token);
