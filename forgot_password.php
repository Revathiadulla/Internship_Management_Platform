...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'step1_lookup') {
        ...
        if (empty($email)) {
            ...
            $sql = "SELECT id, email, full_name, role, phone FROM users WHERE email = '$email' LIMIT 1";
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                $phone = $user['phone'];
                if (empty($phone) && strtolower($user['role']) === 'student') {
                    $student_sql = "SELECT phone FROM student_profiles WHERE user_id = " . intval($user['id']) . " LIMIT 1";
        ...
        if ($method !== 'email' && $method !== 'phone') {
            ...
            if ($method === 'phone' && empty($_SESSION['reset_phone'])) {
                ...
                $insert_sql = "INSERT INTO password_resets (email, code, send_method, expires_at) 
                               VALUES ('" . mysqli_real_escape_string($conn, $email) . "', '$code', '$method', DATE_ADD(NOW(), INTERVAL 15 MINUTE))";
                ...
                if (mysqli_query($conn, $insert_sql)) {
                    if ($method === 'email') {
                        ...
                        if ($email_sent) {
                            ...
                            $error_msg = "Failed to send verification email. Please check your SMTP settings in the .env file. (Details logged in email_notifications.log)";
                        }
                    } else {
                        // Send OTP code via mock SMS
                        $log_file = __DIR__ . "/sms_notifications.log";
                        $sms_msg = "========================================================================\n" .
                                   "[" . date('Y-m-d H:i:s') . "] OUTGOING SMS - STATUS: Sent\n" .
                                   "TO: " . $_SESSION['reset_phone'] . "\n" .
                                   "MESSAGE: Hello " . $_SESSION['reset_name'] . ", your IMP verification code is: $code. Valid for 15 mins.\n" .
                                   "========================================================================\n\n";
                        @file_put_contents($log_file, $sms_msg, FILE_APPEND);
    ...
    elseif ($action === 'step3_verify_otp') {
        ...
        if (empty($code)) {
            ...
            $sql = "SELECT * FROM password_resets 
                    WHERE email = '$esc_email' AND code = '$esc_code' AND expires_at > NOW() 
            ...
            <div class="bg-surface-container-lowest rounded-xl shadow-[0px_1px_3px_rgba(0,0,0,0.1),0px_1px_2px_rgba(0,0,0,0.06)] p-8 md:p-10 border border-outline-variant">
                ...
                <?php if ($step === 1): ?>
                    <div class="text-center mb-6">
                        ...
                        <p class="font-body-md text-body-md text-on-surface-variant mt-2">Enter your email address to recover your account.</p>
                    </div>
                    ...
                    <form action="forgot_password.php" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input name="email" class="w-full pl-10 pr-4 py-2.5 rounded-lg border border-outline outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all font-body-md text-body-md" placeholder="Enter your registered email" type="email" value="<?php echo htmlspecialchars($_POST['email'] ?? ''); ?>" required>
                            </div>
                        </div>
                ...
                <?php elseif ($step === 3): ?>
                    ...
                    <!-- Developer Preview OTP code banner -->
                    <div class="p-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-800 font-medium mb-5">
                        <div class="flex items-center gap-2 font-bold mb-1">
                            <span class="material-symbols-outlined text-sm">construction</span>
                            <span>Dev Environment Preview:</span>
                        </div>
                        <p>
                            <?php if ($_SESSION['reset_method'] === 'email'): ?>
                                Check the mail log: <a href="email_notifications.log" target="_blank" class="underline text-amber-900 font-semibold">[email_notifications.log]</a>
                            <?php else: ?>
                                Check the SMS log: <a href="sms_notifications.log" target="_blank" class="underline text-amber-900 font-semibold">[sms_notifications.log]</a>
                            <?php endif; ?>
                            for the code.
                        </p>
                    </div>
