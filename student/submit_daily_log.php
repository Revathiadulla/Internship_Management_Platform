...
$time_spent = trim($_POST['time_spent'] ?? '0');
...
$time_spent_numeric = floatval($time_spent);
$date_obj = DateTime::createFromFormat('Y-m-d', $log_date);
if (!$date_obj || $date_obj->format('Y-m-d') !== $log_date || $tasks_completed === '' || $focus_level === '' || $time_spent_numeric <= 0) {
    header("Location: {$redirect_base}&error=" . urlencode('Unable to process daily log.'));
    exit();
}
...
if ($mentor_id > 0) {
    ...
    $link = "daily_logs.php?log_id=" . $log_id;
