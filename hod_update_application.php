...
if ($app_id <= 0 || !in_array($action, ['approve', 'reject'])) {
    header("Location: hod_dashboard.php?error=invalid_request");
    exit();
}
...
$res = $stmt->get_result();
if ($res->num_rows === 0) {
    header("Location: hod_dashboard.php?error=not_found");
    exit();
}
$app = $res->fetch_assoc();
...
header("Location: hod_dashboard.php?success=" . $action);
exit();
