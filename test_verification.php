...
$hrId = $conn->insert_id;
// Insert HOD user
$conn->query("INSERT INTO users (full_name, email, role, password) VALUES ('Test HOD', '$hodEmail', 'hod', '')");
...
$conn->query("INSERT INTO internship_applications (user_id, internship_name, status, test_score, aadhaar_status, pan_status, student_type) VALUES ($studentId, 'Test Internship', 'HR Review', 85, 'verified', 'verified', 'pursuing')");
...
$appRes = $conn->query("SELECT * FROM internship_applications WHERE id = $appId");
...
$updateStmt->bind_param('ssssi', $newStatus = 'Forwarded to HOD', $newHodStatus = 'pending', $newFinalStatus = 'pending', $hodToken, $appId);
$updateStmt->execute();
$updateStmt->close();
...
$approveStmt->bind_param('sssi', $finalStatus = 'Selected', $finalHodStatus = 'approved', $finalFinalStatus = 'selected', $appId);
$approveStmt->execute();
$approveStmt->close();
